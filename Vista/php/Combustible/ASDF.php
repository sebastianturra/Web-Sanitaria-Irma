...
$Combustibles = new combustible();ssssddddd
...
/*
      //Tira la cantidad de dias del mes elegido.
      $resultado = $Combustibles->contadordias($anio,$meses);
      $var = $resultado[0]["dias"];
      //echo "<script> alert('Dias del mes".$var."'); </script>"; 
       if($var!="error"){
                //echo "<script> alert('Bien hecho Los dias contados son: ".$var."'); </script>";
                $fechas = array();
                $aumentadordias=1;
                //agrega las fechas del mes al array fechas.
                for($i=0;$i<($var);$i++){
                    $valorfecha=$anio."-".$meses."-".$aumentadordias;
                    $fechas[$i]=$valorfecha;
                    $aumentadordias++;
                }
                //Imprime la fechas del mes del array fechas.
                for($i=0;$i<(count($fechas));$i++){
                 // echo "<script> alert('".$fechas[$i]."'); </script>";
                }
                ...
                */
                ...
                $vehiculos = array();
                $resultadovehiculos = $Combustibles->vehiculomes($meses);
                $contadortrue=0;
                for ($i=0; $i < count($resultadovehiculos); $i++) {
                 $codigovehiculo=$resultadovehiculos[$i]['VEH_CODIGO']; 
                // echo "<script> alert('Mes: ".$meses." codigovehiculo: ".$codigovehiculo."'); </script>";
                    $verificarvehiculo = $Combustibles->validarlistadovehiculo($codigovehiculo,$meses,$anio);
                   // var_dump("Codigo vehiculo:".$codigovehiculo." meses:".$meses." anio:".$anio);
                      if($verificarvehiculo == true){
                        $vehiculos[$contadortrue]=$resultadovehiculos[$i];
                        $contadortrue++;
                       }else{
            ...
            /*  $b=0;
              $facturas = $Combustibles->obtenerfacturas($meses);
              if($facturas=="error"){
                    for ($a=0; $a < $diasmes; $a++){
                          $data[$a]['factura']=0;
                          $data[$a]['GCOMB_CODIGO']=0;
                          $data[$a]['cantidaddesp']=0;
                    }
              }else{
                      $posicionesfactura = Array();
                    for ($i=0; $i < count($facturas); $i++) { //1,2,3,4
                      for ($a=0; $a < $diasmes; $a++) {   //(1 -> 230)
                        if($data[$a]['fecha']==$facturas[$i]['GCOMG_FECHA']){
                        //var_dump("fecha array: ".$data[$a]['fecha']." Y fecha factura: ".$facturas[$i]['GCOMG_FECHA']."bcodigo factura: ".$facturas[$i]['GCOMB_FACTURA']." Codigo detalle: ".$facturas[$i]['GCOMB_CODIGO']);
                          $posicionesfactura[$b]['posicion']=$a;
                          $posicionesfactura[$b]['codigoguiadetalle']=$facturas[$i]['GCOMB_CODIGO'];
                          $b++;                  
                        }else{
                          $data[$a]['factura']=0;
                          $data[$a]['GCOMB_CODIGO']=0;
                    ...
                    $c=0;
                    for ($i=0; $i < count($posicionesfactura) ; $i++) { 
                        $posicion = $posicionesfactura[$i]['posicion'];
                        $data[$posicion]['factura']=$facturas[$c]['GCOMB_FACTURA'];
                        $data[$posicion]['GCOMB_CODIGO']=$facturas[$c]['GCOMB_CODIGO'];
                        $c++;
                    }

                   // for ($i=0; $i < $diasmes; $i++) { 
                   //     $data[$i]['cantidaddesp']= count($posicionesfactura);
                   // }
              }  

               for ($i=0; $i < count($data) ; $i++) {    //Cantidad facturas array 0,1,2
                 //var_dump("Factura ".$facturadesp[$i]['GCOMB_FACTURA']."Codigo ".$facturadesp[$i]['GCOMB_CODIGO']);
                  $b=0;
                  $c=0;
                 $guiasdespacho = $Combustibles->obtenerguiasdespacho($meses,$data[$i]["GCOMB_CODIGO"]);
                  if($guiasdespacho!="error"){
                     for ($a=0; $a < count($guiasdespacho) ; $a++) { //0,1,2,3,4
                      // var_dump("Numero: ".$guiasdespacho[$a]['GDESP_NUMERO']." Litros cargados: ".$guiasdespacho[$a]['GDESP_LCAR']." Codigo: ".$guiasdespacho[$a]['GCOMB_CODIGO']." Tipo Carga: ".$guiasdespacho[$a]['GDESP_TIPOCARGA']);
                       if($guiasdespacho[$a]['GDESP_TIPOCARGA']==2){
                          
                          $data[$i]["despachonumero$b"]=$guiasdespacho[$a]['GDESP_NUMERO'];
                      //    $guiasdespacho[$b]['GDESP_LCAR']=99;
                          $b++;
                       }else{
                          $data[$i]["despachonumero0"]="-";
                        //  $guiasdespacho[$b]['GDESP_LCAR']=33;
                       }
                      } 
                    }else{
                      $data[$i]["despachonumero0"]="-";
           ...
           /*     $f=0;
                ...
                for ($i=0; $i < count($vehiculos); $i++) { 
                  ...
                  for ($a=0; $a < count($fechas) ; $a++) { 
                    $veh_codigo=$vehiculos[$i]['VEH_CODIGO'];
                    $fecha=$fechas[$a]['selected_date'];
                   // var_dump("Vehiculo: ".$veh_codigo." fecha: ".$fecha);
                    $resultado= $Combustibles -> litrosdescuentos($veh_codigo,$fecha);
                   // var_dump($resultado);
                    if($resultado[0]['suma']==NULL){
                      //$resultado[0]['suma']=0;
                     // $arraylitros[$d]=$resultado[0]['suma'];
                    }else{
                      $litrosdescuentos+=$resultado[0]['suma'];
                    }     
                  }
                  $arraylitros[$f]=$litrosdescuentos;
                  $f++;
                }

                for ($i=0; $i < count($arraylitros); $i++) { 
                  $total=$data[0]["total".$i];
                  $descuento=$arraylitros[$i];
                  $operacion=$total-$descuento;
                  $data[0]["total".$i]=$operacion;
                }     */

                  
              
              //var_dump($posicionesfactura);
 //var_dump($data);
                echo json_encode($data);
  }
