...
//Detalle cotización
    ...
    $resultado=$cotizacion->crecotcant($COT_EMPRESA, $COT_FECHA, $COT_TELEFONO, $COT_CONTACTO, $COT_CONDVENTA, $COT_TOTAL, $COT_OBSERVACION, $COT_CONDICIONES, $EST_TIPDETCOTCOD,$COT_NETO,$COT_IVA,$COT_DIRECCION,$COT_TGIRO,$COT_RUT,$COT_CORREO);   
    if($resultado==true){
            ...
            foreach ($_POST["iva"] as $key => $value) {
                        ...
                        if(empty($DCOT_DESCRIPCION)){
                          ...
                          if($resultadocompexterno==true){
                             // echo "<script> alert('Agregado con exito datelle cotización '); </script>";
                            //  echo "<script> $(location).attr('href','listadocombustible.php'); </script>";
                          }else{
                            // echo "<script> alert('Fállo al agregar detalle cotización'); </script>";
                           //  echo "<script> $(location).attr('href','listadocombustible.php'); </script>";
                          } 
                        }
                      }
 ...
 // $listado =  $cotizacion->selectestado($tipocot,$estado,$fechaactual);
  ...
  if(is_null($listado)){
    ...
    if($listado==true){
                echo "<script> alert('COTIZACIÓN ELIMINADA CON EXITO'); </script>";
                echo "<script> $(location).attr('href','listadocotizacion.php'); </script>";
            }else{
            echo "<script> alert('FALLO AL ELIMINAR COTIZACIÓN'); </script>";
            echo "<script> $(location).attr('href','listadocotizacion.php'); </script>";
            }

}else if($_POST["funcion"] == "modificar"){

    $COT_CODIGO = $_POST["codemp"];
    $COT_EMPRESA=$_POST["empnombre"];
    $COT_FECHA=$_POST["feccot"];
    $COT_TELEFONO=$_POST["telcot"];
    $COT_CONTACTO=$_POST["concot"];
    ...
    $resultado=$cotizacion->actualizarcotizacion($EST_PROCODIGO,$EST_TIPDETCOTCOD,$EST_COTCODIGO,$COT_EMPRESA, $COT_FECHA,$COT_TELEFONO,$COT_CONTACTO,$COT_CONDVENTA,$COT_NETO,$COT_IVA, $COT_TOTAL, $COT_OBSERVACION,$COT_CONDICIONES,$COT_CODIGO,$COT_DIRECCION,$COT_TGIRO,$COT_RUT,$COT_CORREO);   
    if($resultado==true){ 
            foreach ($_POST["iva"] as $key => $value) {
                        $DCOT_DESCRIPCION=$_POST["descot"][$key];
                        $DCOT_CBFCOT=$_POST["cbfcot"][$key];
        ...
        echo "<script> alert('Cotización Modificada Con Exito'); </script>";
        echo "<script> $(location).attr('href','listadocotizacion.php'); </script>"; 
    }else{
       echo "<script> alert('Fallo Al Modificar Cotización'); </script>";
       echo "<script> $(location).attr('href','listadocotizacion.php'); </script>"; 
    }  
}else{
